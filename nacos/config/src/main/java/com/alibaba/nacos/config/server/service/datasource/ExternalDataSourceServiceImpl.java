    ...
    public void init() {
        ...
        if (PropertyUtil.isUseExternalDB()) {
            try {
                ...
                FATAL_LOG.error("[ExternalDataSourceService] 加载jdbc出现了异常", e);
    ...
    public synchronized void reload() throws IOException {
        try {
            dataSourceList = new ExternalDataSourceProperties()
                    .build(EnvUtil.getEnvironment(), new ExternalDataSourceProperties.Callback<HikariDataSource>() {
                        @Override
                        public void accept(HikariDataSource dataSource) {
